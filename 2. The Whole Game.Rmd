...
Verify that I'm using the same version of devtools as the authors:

```{r Verify this is the same system of devtools as the authors}

packageVersion("devtools")

```

I'm using version 2.4.5, the same as the version used by the authors.
...
You can see the glorious result we are working towards by visiting regexcite on GitHub: [https://github.com/jennybc/regexcite](you can see the glorious result we are working towards by visiting regexcite on GitHub: https://github.com/jennybc/regexcite)

## 2.5 `create_package()`

Call `create_package()` to initialize a new package in a directory on your computer. `create_package()` will automatically create that directory if it doesn’t exist yet (and that is usually the case).
